...
if (-not $versionLine) {
    Write-Host "❌ Could not determine version from pyproject.toml" -ForegroundColor Red
    exit 1
}
...
if ($dockerInfo -notmatch "Username:\s*$DOCKER_USER") {
    Write-Host "⚠️  Not logged in to Docker Hub as $DOCKER_USER" -ForegroundColor Yellow
